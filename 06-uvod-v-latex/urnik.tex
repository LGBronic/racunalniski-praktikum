...
(skoraj) vedno, da jih ne bo prekršil. Tak način uteževanja pride v poštev
...
omejitve so bile kršene ter kolikokrat se kršitev pojavi. S \begin{math}
	k_n
\end{math} označimo število
kršitev omejitve \(n\), z \(u_n\) pa njeno utež. Z naslednjo formulo izračunamo
kakovost urnika:
\begin{displaymath}
k_1 \cdot u_1 + k_2 \cdot u_2 + \cdots k_{n-1} \cdot u_{n-1} + kn \cdot un
\end{displaymath}
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   \(j\)-tem koraku označimo s \(\text{predavalnice}_{j}\).
...
% V spodnjem odstavku naj bodo črke G, U in I v `\1' podčrtane.
Sledili smo receptu in s tem izdelali, treba ga je le še ustrezno postreči
